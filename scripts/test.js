...
assert.ok(!fs.existsSync(path.join(root, 'index.html')), 'root index.html must not exist on the desktop-only main branch')
assert.ok(!fs.existsSync(path.join(root, 'showcase.html')), 'root showcase.html must not exist on the desktop-only main branch')
...
assertNotContains('app/app.js', ["localStorage.setItem('nono_tasks'", "localStorage.setItem('nono_stats'", "localStorage.setItem('nono_mood'", "localStorage.setItem('nono_last_activity'", "localStorage.setItem(HERMES_MEMORY_KEY", "localStorage.getItem(HERMES_MEMORY_KEY", "localStorage.setItem(STATS_KEY"])
assertNotContains('app/app.js', ["if(!IS_ELECTRON){\n    if(key) cfg.k=key"])
assertNotContains('app/app.js', ['anthropic-dangerous-direct-browser-access', "Authorization':`Bearer ${cfg.k}`", 'hermesAgentHeaders'])
assertContains('package.json', [`"version": "${pkg.version}"`, `"artifactName": "孬孬-Setup-\${version}.\${ext}"`])
